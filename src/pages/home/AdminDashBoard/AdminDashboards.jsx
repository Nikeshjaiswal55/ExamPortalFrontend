...
import {
  ...
  useGetTop3AssissmentStudentsQuery,
  useGetTop5AssessmentOfOrgIdQuery,
  useGetTop5StudentsByOrgIdQuery,
  useGetTotalStudentAndAssessementByOrgIdQuery,
} from '../../../apis/Service';
    ...
    <>
      <div className="col-12  p-1 m-0">
        <div className=" h-100 d-flex align-items-center gap-2  border m-1 p-1 rounded-2">
          <span
            className={` h-100 w-25 d-flex justify-content-center align-items-center rounded-2   elevation-1 `}
          >
            <div
  ...
  return (
    <>
      <Accordion.Item eventKey={index} className=" border-0  border-bottom ">
        <Accordion.Header className=" border border-white">
          {' '}
          <div className="row w-100  gap-2 gap-lg-0 fw-bold ">
        ...
        <Accordion.Body className="px-2 py-0 w-100">
          <div className="row w-100">
            ...
            {TopStudent?.length ? null : (
              <p> Actively no student Toper present </p>
            )}
          </div>
        </Accordion.Body>
      </Accordion.Item>
...
export const AdminDashboard = () => {
  ...
  const {
    data: totalStudentsAndAssessment,
    isLoading: totalStudentsAndAssessmentLoading,
    isError: totalStudentsAndAssessmentError,
  } = useGetTotalStudentAndAssessementByOrgIdQuery(organizationId);
  ...
  const {
    data: topRankers,
    isLoading: topRankersLoading,
    isFetching: topRankersFetching,
    isError: topRankersError,
  } = useGetTop5StudentsByOrgIdQuery(organizationId);
  const {
    data: topAssessmentOrgRanking,
    isLoading: topAssessmentOfOrgLoading,
    isError: topAssessmentOfOrgIsError,
  } = useGetTop5AssessmentOfOrgIdQuery(organizationId);
  ...
  function getTopRankeAssessmentsDataOrg() {
    ...
    topAssessmentOrgRanking?.forEach((value, index) => {
      topRanker[index] = {
        ...
        assesment_Name: value?.assesment_Name,
      };
    });
    return topRanker;
  }
  ...
  const info = [
    {
      ...
      children: (
        <>
          {' '}
          {totalStudentsAndAssessmentLoading ? (
            <Spinner
              size="sm"
              variant="primary"
              className=" ms-2"
    ...
    },
    {
      ...
      onViewClick: () => {
        navigate(`/admin/total-student/${organizationId}`);
      },
              ...
              <div className="col-12 col-md-6 d-flex  p-0 px-md-1    flex-column align-items-center    justify-content-center rounded-3 admin-top-rank-container">
                <div className=" w-100 h-100 mt-1 m-md-0 bg-white rounded-3 ">
                  ...
                  <div className=" m-0 p-0  w-100" style={{ height: '90%' }}>
                    {topAssessmentOfOrg && topAssessmentOfOrg?.length > 0 && (
                      <div className=" chart-parent w-100 h-100    d-flex    align-items-center   rounded-3 py-3 justify-content-center ">
                        <BarChart
                          assessemnt1={
                            topAssessmentOfOrg?.[0]?.assesment_Name ??
                            'Not present'
                          }
                          assessemnt2={
                            topAssessmentOfOrg?.[1]?.assesment_Name ??
                            'Not present'
                          }
                          assessemnt3={
                            topAssessmentOfOrg?.[2]?.assesment_Name ??
                            'Not present'
                          }
                          assessemnt4={
                            topAssessmentOfOrg?.[3]?.assesment_Name ??
                            'Not present'
                          }
                          assessemnt5={
                            topAssessmentOfOrg?.[4]?.assesment_Name ??
                            'Not present'
                          }
                    ...
                    {topAssessmentOfOrg &&
                      topAssessmentOfOrg?.length == 0 &&
                      !topAssessmentOfOrgIsError &&
                      !topAssessmentOfOrgLoading && (
                        <div className=" chart-parent w-100 h-100    d-flex    align-items-center   rounded-3 py-3 justify-content-center ">
                          <BarChart
              ...
              <div className="row w-100 h-100  chart-box   p-0  m-0 ">
                <div className="col-12 col-md-4 px-0 px-md-1  h-auto overflow-auto  d-flex   align-items-center    justify-content-start   rounded-3 overflow-auto top-rank-student-container">
                  <div className=" w-100  h-100  d-flex flex-column bg-white   align-items-center    justify-content-start   rounded-3">
                    <h5 className=" ps-3 pt-2 align-self-start d-flex justify-content-start align-items-center">
                      {' '}
                      Top Ranking Student{' '}
                      {topRankersLoading && (
                        <>
                    ...
                    {topRankersError || topStudentOfOrg?.length == 0 ? (
                      <div className=" chart-parent w-100 d-flex  h-100  align-items-center  rounded-3 py-3  justify-content-center ">
                        <SolidGauge
                          assessemnt1={'Not present'}
                          assessemnt2={'Not present'}
                          assessemnt3={'Not present'}
                    ...
                    {topStudentOfOrg &&
                    topStudentOfOrg?.length &&
                    !topRankersLoading &&
                    !topRankersFetching ? (
                      <div className=" chart-parent w-100 d-flex  h-100  align-items-center  rounded-3 py-3  justify-content-center ">
                        <SolidGauge
                          assessemnt1={
                            topStudentOfOrg?.[0]?.name ?? 'Not present'
                          }
                          assessemnt2={
                            topStudentOfOrg?.[1]?.name ?? 'Not present'
                          }
                          assessemnt3={
                            topStudentOfOrg?.[2]?.name ?? 'Not present'
                          }
                          assessemnt4={
                            topStudentOfOrg?.[3]?.name ?? 'Not present'
                          }
                          assessemnt5={
                            topStudentOfOrg?.[4]?.name ?? 'Not present'
                          }
                ...
                <div className="col-12 col-md-8 h-100  p-0 pe-lg-1 d-flex  align-items-center mt-2 mt-md-0    justify-content-center rounded-3 overflow-auto active-assessment-container">
                  <div className=" chart-parent w-100 h-100 m-md-0 d-flex  mb-0  align-items-start  rounded-3  justify-content-center bg-white ">
                    {
                      <div className="row m-0  d-flex flex-column justify-content-start bg-white  rounded-3 w-100 h-100 overflow-auto  p-0   ">
                        <div className="col-12 w-100  p-md-0 m-0   fw-bold ">
                          <h5 className=" ps-4 pt-3 d-flex align-items-start justify-content-start">
                            {' '}
                            Active Assessment{' '}
                            {topAssessmentOfOrgLoading && (
                              <Spinner
                        ...
                        <Accordion className="col-12 w-100 h-75 p-0  ">
                          {data?.data &&
                            data?.data?.map((value, index) => {
                              if (value.is_Active == 'true') {
                                return (
                                  <TopStudentAcordianItem
                                    key={index}
                                    index={index}
                                    assessmentId={value?.paperId}
                                    assessmentName={value?.assessmentName}
                                    examDate={'28-09-2033'}
                                    examDuration={TimeFormate(value?.examDuration)}
                                  />
                                );
                              } else {
                                return null;
