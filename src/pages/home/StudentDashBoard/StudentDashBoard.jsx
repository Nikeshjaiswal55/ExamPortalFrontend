...
import {useGetAllAssissmentOnstudentPageQuery,useGetPassedAssessmentByStudentIdQuery,useGetTop5AssesmentScoreByStudentIdQuery} from '../../../apis/Service';
...
export const StudentDashBoard = () => {
  ...
  const {
    ...
    isFetching: isAssignmentFetching} = useGetTop5AssesmentScoreByStudentIdQuery(userId);

  const {
    data: passedAssessment,
    isLoading: passedAssessmentLoading} = useGetPassedAssessmentByStudentIdQuery(userId);
  // const top15Students = [
  //   {
  //     assesment_Name: "yes",
  //     percentage: 10
  ...
  const info = [
    {
      ...
      children: <> {isAssignmentLoading ? <Spinner size='sm' variant='primary' className=' ms-3' animation="border" /> : null} </>,
      iconClassName: 'bg-warning',
      onViewClick: () => {navigate(path.ShowAllAssessmentToStudent.path)},
    ...
    },
    {
      ...
      colClassName: " w-100",
      children: <> {passedAssessmentLoading ? <Spinner size='sm' variant='primary' className=' ms-3' animation="border" /> : null} </>,
      onViewClick: () => {""},
    },
  ];
  useEffect(() => {
        ...
        <div className="row w-100   chart-box    p-1 m-0 h-50" >
          <div className="col-12 d-flex  p-0 px-1    flex-column align-items-center    justify-content-center rounded-3">
            <div className=' w-100 h-100 bg-white rounded-3 '>
              <h5 className=' ps-5 pt-2 m-0 d-flex justify-content-start align-items-center'> Top Rank Assessment    {top15StudentsLoading && <Spinner className=' text-primary ms-3' animation="border" size='sm' />}</h5>
              <div className=' m-0 p-0  w-100' style={{height: "90%"}}>
                {top15Students && top15Students?.length > 0 && <div className=' chart-parent w-100 h-100    d-flex    align-items-center   rounded-3 py-3 justify-content-center '>
                  <BarChartStudentDashboard
                    studentData={top15Students}
                  />
                </div>}
                {top15Students && top15Students?.length == 0 && !top15StudentsIsError && !top15StudentsLoading && <div className='  d-flex w-100 flex-column'>
                  ...
                  <div className="row m-0  d-flex justify-content-between bg-white  rounded-3 w-100 h-100  p-2   ">
                    ...
                    {isAttemptedAssessment == 0 && !isAssignmentLoading && !isAssignmentFetching && !isAssignmentError ? <><div className='  d-flex w-100 h-100 justify-content-center align-items-center  flex-column'>
                      <div className=' d-flex justify-content-around w-100 '>
                        <img src={noAssessmentFound} alt="error" width={"200px"} height={"200px"} />
                      </div>
