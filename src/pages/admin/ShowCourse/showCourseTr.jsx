...
import {useDeleteCourseMutation,useUpdateCourseMutation} from '../../../apis/Service';
...
export function ShowCourseTr(props) {
    ...
    async function updateData(courseName,id) {
        if(accessToken) {
            if(user) {
                ...
                console.log("details",updateCourseDetail);
                updateCourse(updateCourseDetail).then((resp) => console.log(resp)).catch((err) => alert(JSON.stringify(err)));
    ...
    async function handleDeleteCourse(id) {
        if(accessToken) {
            deleteCourse({accessToken,id}).then(() => console.log(responseDelete)).catch((err) => alert("delete course error ",JSON.stringify(err),))
        }
        else {
            alert("user access token not present ");
        }
    ...
    return (
        ...
        <>
            <tr >
                ...
                <td className='custom-table-td-width' > <RiDeleteBin6Line className=' mx-2'
                    onClick={handleDelete} color='red'
            ...
            >
                <Modal.Header >
                    <Modal.Title id="contained-modal-title-vcenter"> Edit Course Name </Modal.Title>
                </Modal.Header>
            ...
            >
                ...
                <Modal.Footer>
                    ...
                    <Button variant="danger" type='submit' onClick={() => handleDeleteCourse(props.courseId)} >Delete</Button>
                </Modal.Footer>
            </Modal>}
        </>
    )
