...
export function SnsSvsDashboard() {
    ...
    useEffect(() => {
        if (selectedDistrict && selectedDistrict !== 'all') {
            ...
            axios.get(`https://district-seven.vercel.app/api/districts/${selectedDistrict}/tehsils`)
                .then((response) => {
    ...
    useEffect(() => {
        ...
        if (selectedTehsil && selectedTehsil !== 'all') {
            filteredData = filteredData.filter((item) => item?.tehsil === selectedTehsil);
        }
    ...
    const exportExcel = () => {
        const tableData = selectedPaper === 'all' ? filteredData : paperStudent?.students || [];

        const formattedData = selectedPaper === 'all'
    ...
    useEffect(() => {
        if (selectedPaper && selectedPaper !== 'all') {
            axios.get(`https://examsapi.ssism.org/getFullDetails?paperId=${selectedPaper}`)
                .then((response) => {
    ...
    const filteredByVillage = selectedVillage === 'all' ? filteredData : filteredData.filter(student => student?.village === selectedVillage);
    ...
    const handleTehsilClick = (tehsil) => {
        ...
        const schoolsInTehsil = filteredData.filter(student => student?.tehsil === tehsil);
                ...
                <>
                    ...
                    <div className="dashboard-summary d-flex justify-content-between">
                        <div className="district-summary">
                            <h6 style={{ fontSize: '16px', fontWeight: 'bold', color: '#333' }}><strong>District-wise Summary:</strong></h6>
                            <div className="d-flex flex-wrap gap-3">
                                {Object.entries(districtCount).map(([district, count]) => (
                                    <div key={district} className="card" style={{
                                        width: '120px', backgroundColor: '#eaf7ff', color: '#0056b3',
                                        padding: '8px', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)',
                                        fontSize: '12px', textAlign: 'center', height: '100px'
                                    }}>
                                        <h6 style={{ fontSize: '14px', fontWeight: 'bold' }}>{district}</h6>
                        ...
                        <div className="tehsil-summary">
                            <h6 style={{ fontSize: '16px', fontWeight: 'bold', color: '#333' }}><strong>Tehsil-wise Summary:</strong></h6>
                            <div className="d-flex flex-wrap gap-3">
                                {Object.entries(tehsilCount).map(([tehsil, count]) => (
                                    <div 
                                        ...
                                        style={{
                                            width: '120px', backgroundColor: '#d4f8d4', color: '#28a745',
                                            padding: '8px', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)',
                                            fontSize: '12px', textAlign: 'center', height: '100px', cursor: 'pointer'
                                        }}
                                    >
                                        <h6 style={{ fontSize: '14px', fontWeight: 'bold' }}>{tehsil}</h6>
                                        <p style={{ fontSize: '12px' }}>{count} students</p>
                                    </div>
                                ))}
                            </div>
                    ...
                    {selectedTehsil && selectedTehsil !== 'all' && (
                        <div className="schools-summary" style={{ marginTop: '20px' }}>
                            <h6 style={{ fontSize: '16px', fontWeight: 'bold', color: '#333' }}><strong>Schools in {selectedTehsil}:</strong></h6>
                            <div className="d-flex flex-wrap gap-3">
                                {selectedTehsilSchools.map((school, index) => (
                                    <div 
                                        ...
                                        className="card" 
                                        style={{
                                            width: '200px', backgroundColor: '#fff3cd', color: '#856404',
                                            padding: '8px', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)',
                                            fontSize: '12px', textAlign: 'center', height: '80px'
                                        }}
                                    >
                                        <h6 style={{ fontSize: '14px', fontWeight: 'bold' }}>{school.school}</h6>
                                        <p style={{ fontSize: '12px' }}>{school.count} students</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <button onClick={exportExcel} className="btn btn-primary">Export to Excel</button>
                </>
            )}
        </>
    );
